...
$(document).ready(function(){
  ...
  $(document).ready(function(){
    ...
    $('.btn_link').click(function() {
      // 복사할 텍스트 가져오기
      var textToCopy = "https://www.shieldhero.itoxi.co.kr"; // 여기에 복사하고 싶은 링크를 넣어주세요
...
function allowScroll() {
  $('body').off('scroll touchmove mousewheel'); // 스크롤 이벤트 해제
}
...
// 팝업 닫기 함수 (여기서 스크롤을 허용)
function closePopup(popupId) {
  document.getElementById(popupId).style.display = 'none';
  allowScroll(); // 팝업 닫힐 때 스크롤 허용
}

// 예시: 팝업 닫기 버튼에 이벤트 추가 (각 팝업에 맞는 닫기 버튼을 추가해야 합니다)
document.querySelectorAll('.btn-pop-ok').forEach(function(button) {
  button.addEventListener('click', function() {
    closePopup(button.dataset.popup); // data-popup 속성에 팝업 ID를 설정
  });
});
  ...
  button.addEventListener('click', function() {
    ...
    switch (this.classList[1]) {
      case 'card_num_1':
        ...
        이른바 &quot;동료&quot;보다는, 차라리 &quot;배신할 수 없는&quot;……
        <br /><br />
        ● 이럴 수가! 전 세계 사람들이 터무니없는 죄명으로 저를 비난하고 있어요! 나는 분명
        무고한데, 왜 아무도 나를 믿지 않는 거죠! …… 라프타리아, 방금 뭐라고 했어……
        너는 나를 믿는다고……? 이건 내가 계속 듣고 싶었던 말이에요. 아니,
        라프타리아는 줄곧……
        <br /><br />
        ● 이세계에서 온…… 용사? 그들의 목표가…… 나라고!? 이
      ...
      case 'card_num_2':
        ...
        여기서 나갈 수 있을 거야! 리파나? 리파나? 나를 떠나지 말아줘……
        <br /><br />
        ● 너 같은 비열한 인간, 내가 언제 너한테 구해달라고 했냐? &quot;창의 용사&quot;라는
        이름을 믿고 자만하지 마, 너는 나오후미 님을 이해하고 있기는 해? 나오후미 님은
        내가 원하지 않는 일을 강요하지 않아, 피곤하면 쉬게 해줘…… 너는 병에 걸려 언제
        죽을지 모르는 하인을 돕겠다고 할 수 있겠어? 너는 할 수 있겠어?
        <br /><br />
        ● 나는 이 세상에서 나오후미 님을 가장 잘 아는 사람이야, 나오후미 님의 따뜻함을 알아!
        ...
        검, 어떤 고난이 닥쳐도 나오후미 님을 따를 거야!
        <br /><br />
        ● 요즘 필로가 정말로 나오후미님에게 너무 들러붙어! 메르티도 그래! 나오후미 님은 정말…… 너무 둔해! 그게 또
    ...
    const textarea = document.querySelector('.textarea_pop_char');
